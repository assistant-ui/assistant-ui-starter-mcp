 ...
 * - id: Optional todo ID
 */
.post("/", zValidator("json", createTodoSchema.pick({ text: true, userId: true, id: true })), async (c) => {
  const client = postgres(c.env.DATABASE_URL, { prepare: false });
  const db = createDatabase(c.env.DATABASE_URL);
 ...
 */
.put("/:id", zValidator("json", updateTodoSchema.pick({ text: true, completed: true }).partial()), async (c) => {
  const client = postgres(c.env.DATABASE_URL, { prepare: false });
  const db = createDatabase(c.env.DATABASE_URL);
  const id = c.req.param("id");
  const todoData = c.req.valid("json");
