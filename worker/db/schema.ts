...
export const todos = pgTable('todos', {
  ...
  userId: uuid('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }), // Foreign key to users table
  created_at: timestamp('created_at').notNull().defaultNow(),
  updated_at: timestamp('updated_at').notNull().defaultNow(),
});
...
export const todoQuerySchema = z.object({
  ...
  sortBy: z.enum(['text', 'completed', 'created_at', 'updated_at']).optional().default('text').describe("Field to sort by"),
...
export const userQuerySchema = z.object({
  ...
  sortBy: z.enum(['username', 'id', 'created_at', 'updated_at']).optional().default('username').describe("Field to sort by"),
  sortOrder: z.enum(['asc', 'desc']).optional().default('asc').describe("Sort order"),
  limit: z.string()
    .optional()
    .default("50")
...
export const autoCreateUserSchema = z.object({
  ...
  username: z.string().min(1).max(255).optional().describe("Optional username, will default to user ID if not provided"),
});
