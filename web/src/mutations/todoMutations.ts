 ...
 */
function isElectricCollection(
  collection: Collection<any>
): collection is ElectricCollection<any> {
  return (
    'awaitTxId' in collection &&
    typeof (collection as any).awaitTxId === 'function'
  );
}
...
async function collectionSync(
  ...
  if (txid && isElectricCollection(mutation.collection)) {
    ...
    if (
      'refetch' in mutation.collection &&
      typeof mutation.collection.refetch === 'function'
    ) {
  ...
  try {
    ...
    switch (mutation.type) {
      ...
      case 'update': {
        ...
        response = await todoApi.update(
          original.id as string,
          changes as UpdateTodoInput
        );
        break;
      }
