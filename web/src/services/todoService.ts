...
export class TodoApiError extends Error {
  ...
  constructor(
    ...
    const message =
      errorResponse.message || errorResponse.error || defaultMessage;
    super(message);
...
export type TodosGetQueryParams = Parameters<
  typeof client.api.todos.$get
>[0]['query'];
export type UserTodosGetQueryParams = Parameters<
  (typeof client.api.users)[':userId']['todos']['$get']
>[0]['query'];
export type TodosPostJsonBody = Parameters<
  (typeof client.api.todos)['$post']
>[0]['json'];
export type TodoPutJsonBody = Parameters<
  (typeof client.api.todos)[':id']['$put']
>[0]['json'];
...
export const updateSortParams = (
  newParams: Partial<typeof DEFAULT_SORT_PARAMS>
) => {
...
export const isApiError = (error: unknown): error is TodoApiError =>
