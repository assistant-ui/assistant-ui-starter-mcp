...
server.tool(
  ...
  "Updates an existing todo item's text or completion status",
  {
    ...
    text: z
      ...
      .max(1000)
      .optional()
      .describe('The new text for the todo.') as any,
    completed: z
      .boolean()
      .optional()
      .describe('The new completion status.') as any,
  },
  {
    title: 'Update Todo',
    readOnlyHint: false,
  ...
  async (args) => {
    ...
    try {
      ...
      if (Object.keys(updates).length === 0) {
        return {
          content: [
            { type: 'text', text: 'No updates provided for the todo.' },
          ],
        };
      }
...
server.tool(
  ...
  'Retrieves a list of todos for the current user with filtering, sorting, and pagination options',
  {
    ...
    search: z
      ...
      .optional()
      .default('')
      .describe('Search text within todo text') as any,
  },
  {
    title: 'Get Todos',
    readOnlyHint: true,
...
server.tool(
  ...
  'Updates the default sorting and filtering criteria for todo queries',
  {
    ...
    completed: z
      .boolean()
      .optional()
      .describe('Filter by completion status') as any,
  ...
  async (args) => {
    try {
      ...
      if (updates.sortOrder)
        descriptions.push(`in ${updates.sortOrder}ending order`);
      if (updates.completed !== undefined)
        descriptions.push(
          `showing ${updates.completed ? 'completed' : 'incomplete'} todos only`
        );
      if (updates.search)
        descriptions.push(`filtering by search: "${updates.search}"`);
