...
import { CheckCircle, ChevronDown, ChevronRight, FileText, Play, Server, Terminal, Wrench, XCircle } from "lucide-react";
...
function formatMcpError(error: any): { title: string; description: React.ReactNode } {
...
export default function McpServer() {
  ...
  const callTool = async (toolName: string, data: any) => {
    ...
    try {
      const result = await mcpClient.callTool({ name: toolName, arguments: data });
      ...
      const resultText = typeof result === 'object' 
        ? JSON.stringify(result, null, 2).substring(0, 500) + (JSON.stringify(result).length > 500 ? '...' : '')
        : String(result);
      ...
      toast.error(`${toolName} failed: ${title}`, {
        ...
        action: {
          label: "Retry",
          onClick: () => {
            const form = document.querySelector(`[data-tool-form="${toolName}"]`) as HTMLFormElement;
            if (form) {
              form.requestSubmit();
            }
          }
      ...
      {capabilities?.tools && (
        <Card className="p-3">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="font-medium text-sm">Tools ({mcpTools.length})</span>
          </div>
      ...
      {capabilities?.resources && resources.length > 0 && (
        <Card className="p-3">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="font-medium text-sm">Resources ({resources.length})</span>
          </div>
          ...
          <div className="space-y-2">
            {resources.map((resource) => (
              ...
              >
                <div className="font-medium text-xs truncate">{resource.name || resource.uri}</div>
                {resource.description && (
                  <div className="text-muted-foreground text-xs truncate mt-1">{resource.description}</div>
                )}
              </Card>
            ))}
          </div>
...
function ToolCard({ tool, isExpanded, onToggle, onCall, isCalling, schema }: ToolCardProps) {
  ...
  return (
    <Card className="border">
      ...
      {isExpanded && (
        <CardContent className="p-2 pt-0 space-y-3"> {/* Increased space-y for better separation */}
          <Form {...form}>
            <form
            ...
            >
              ...
              {tool.inputSchema?.properties && Object.keys(tool.inputSchema.properties).length > 0 && (
                <div className="space-y-2"> {/* Added container for parameters */}
                  {Object.entries(tool.inputSchema.properties).map(([paramName, paramSchema]: [string, any]) => (
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="text-xs font-medium"> {/* Added font-medium */}
                            {paramName}
                          ...
                          {paramSchema.description && (
                            <FormDescription className="text-xs">{paramSchema.description}</FormDescription>
                          )}
                          <FormMessage className="text-xs" /> {/* Added text-xs */}
                        </FormItem>
                      )}
                    />
                  ))}
              ...
              >
                {isCalling ? (
                  <>
                    ...
                    <span>Calling...</span> {/* Removed text-xs here, already on button */}
                  </>
                ) : (
                  <>
                    <Play className="h-3 w-3 mr-1" />
                    <span>Call</span> {/* Removed text-xs here, already on button */}
                  </>
                )}
              </Button>
            </form>
