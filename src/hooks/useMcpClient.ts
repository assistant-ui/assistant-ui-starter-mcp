// src/lib/mcp/useMcpClient.ts  
import { BrowserClientTransport } from '@modelcontextprotocol/sdk/client/browser.js';
...
export function useMcpClient({  
  ...
  const fetchInitialData = useCallback(async (signal: AbortSignal) => {  
    if (!mcpClient) {  
      ...
      const clientInstance = new Client({ name: clientName, version: clientVersion });  
      try {  
