...
const server = new McpServer({
  ...
  instructions: "You are a helpful assistant that can create, update, and delete todos for the current user.",
});
  ...
  async (args) => {
    ...
    try {
      ...
      if (Object.keys(updates).length === 0) {
        return { content: [{ type: 'text', text: "No updates provided for the todo." }] };
      }
...
server.tool(
  ...
  async () => {
    ...
    try {
      ...
      console.error("MCP Tool Error:", error);
      const errorMessage = isApiError(error) ? getErrorMessage(error) : "Failed to delete all todos";
      return { content: [{ type: 'text', text: `Error: ${errorMessage}` }] };
    }
  }
);
...
server.tool(
  ...
  {
    completed: z.boolean().optional().default(false).describe("Filter todos by completion status") as any,
    sortBy: z.enum(['text', 'completed', 'created_at', 'updated_at']).optional().default('text').describe("Field to sort by") as any,
    sortOrder: z.enum(['asc', 'desc']).optional().default('asc').describe("Sort order") as any,
    limit: z.number().int().positive().max(100).optional().default(50).describe("Maximum number of todos to return") as any,
    offset: z.number().int().min(0).optional().default(0).describe("Number of todos to skip") as any,
...
server.tool(
  ...
  {
    sortBy: z.enum(['text', 'completed', 'created_at', 'updated_at']).optional().describe("Field to sort todos by") as any,
  ...
  async (args) => {
    try {
      ...
      if (updates.completed !== undefined) descriptions.push(`showing ${updates.completed ? 'completed' : 'incomplete'} todos only`);
      ...
      console.error("MCP Tool Error:", error);
      const errorMessage = isApiError(error) ? getErrorMessage(error) : "Failed to update sort criteria";
      return { content: [{ type: 'text', text: `Error: ${errorMessage}` }] };
    }
  }
);
...
server.tool(
  ...
  async () => {
    try {
      ...
      console.error("MCP Tool Error:", error);
      const errorMessage = isApiError(error) ? getErrorMessage(error) : "Failed to get sort criteria";
      return { content: [{ type: 'text', text: `Error: ${errorMessage}` }] };
    }
  }
);
...
server.tool(
  ...
  async () => {
    try {
      resetSortParams();
      return { content: [{ type: 'text', text: "Sort criteria reset to defaults" }] };
    } catch (error) {
      console.error("MCP Tool Error:", error);
      const errorMessage = isApiError(error) ? getErrorMessage(error) : "Failed to reset sort criteria";
      return { content: [{ type: 'text', text: `Error: ${errorMessage}` }] };
    }
  }
);
