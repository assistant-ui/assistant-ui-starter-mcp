 ...
 */
export class BrowserClientTransport implements Transport {
  ...
  constructor(
    globalNamespace: string = 'mcp',
    opts?: BrowserClientTransportOptions
  ) {
    this._globalNamespace = globalNamespace;
    this._reconnectionOptions =
      opts?.reconnectionOptions ?? DEFAULT_BROWSER_RECONNECTION_OPTIONS;
  ...
  async send(
    ...
    if (!this.isConnected || !this._port) {
      ...
      if (options?.resumptionToken) {
        ...
        if (!this.isConnected || !this._port) {
          throw new Error(
            'Not connected after attempting reconnection with token.'
          );
        }
      } else if (
  ...
  async terminateSession(): Promise<void> {
    ...
    try {
      // Clear server-side events for this client
      const windowWithMcp = window as MCPWindow;
      const mcp: MCPBrowserInterface | undefined =
        this._globalNamespace === 'mcp'
          ? windowWithMcp.mcp
  ...
  static isServerAvailable(namespace: string = 'mcp'): boolean {
    ...
    const mcp: MCPBrowserInterface | undefined =
      namespace === 'mcp'
        ? windowWithMcp.mcp
        : (windowWithMcp as any)[namespace];
