...
export class ExtensionClientTransport implements Transport {
  ...
  private _setupMessageHandler(): void {
    this.bridge.onMessage((resp: BridgeResponse) => {
      ...
      try {
        ...
        if (!this.isConnected) {
          console.warn(
            'ExtensionClientTransport: Received message while not connected',
            data
          );
          return;
        }
        ...
        if (
          ...
          console.warn(
            'ExtensionClientTransport received unknown message type:',
            data
          );
        }
      } catch (err) {
  ...
  private async _connectWithRetry(resumptionToken?: EventId): Promise<void> {
    return new Promise<void>((resolve, reject) => {
      ...
      console.log(
        ...
        resumptionToken
          ? `with resumption token ${resumptionToken}`
  ...
  async send(message: JSONRPCMessage, _?: TransportSendOptions): Promise<void> {
    ...
    if (this._startPromise) {
      console.log(
        'ExtensionClientTransport: Waiting for initial connection before send.'
      );
      try {
